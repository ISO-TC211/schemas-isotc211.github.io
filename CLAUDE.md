...
**schemas.isotc211.org** — an interactive schema browser site for ISO/TC 211 XML schemas. The site consists of:
- A **Jekyll site** using `jekyll-theme-isotc211` gem with Vite + Tailwind CSS v4 frontend
- Per-standard **interactive SPA browsers** generated by lutaml-xsd
...
bundle install
BUNDLE_GEMFILE=Gemfile.lutaml bundle install
npm install
...
lutaml-xsd requires liquid ~> 5.0; Jekyll requires liquid ~> 4.0. They cannot coexist in one Gemfile:
- **`Gemfile`** — Jekyll + jekyll-theme-isotc211 + jekyll-vite (for `bundle exec jekyll build/serve`)
- **`Gemfile.lutaml`** — lutaml-xsd + lutaml-model (for `BUNDLE_GEMFILE=Gemfile.lutaml bundle exec lutaml-xsd ...`)

The Makefile uses `BUNDLE_GEMFILE=Gemfile.lutaml` automatically for lutaml-xsd commands.
...
site/                 → SPA HTML output from lutaml-xsd
...
Gemfile               → Jekyll deps (jekyll, jekyll-theme-isotc211, jekyll-vite, jekyll-feed, jekyll-redirect-from)
Gemfile.lutaml        → lutaml-xsd deps (lutaml-xsd, lutaml-model, moxml, canon)
...
- **Ruby** (3.3) with Bundler — Jekyll site + lutaml-xsd schema browser generation
- **Node.js** (20) with Vite + Tailwind CSS v4 — frontend assets (via jekyll-vite)
- **lutaml-xsd** (~> 1.1) — parses XSD → LXR package → Vue SPA HTML
- **jekyll-theme-isotc211** — shared Jekyll theme gem (at `../jekyll-theme-isotc211/`)

## Deployment

GitHub Actions workflow (`.github/workflows/build_deploy.yml`) builds on push to `main` and deploys `_site/` to GitHub Pages.
